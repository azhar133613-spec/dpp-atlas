...
// ── PUBLIC DPP PASSPORT PAGE /dpp/[gtin] ─────────────────────
w('app/dpp/[gtin]/page.tsx', `
...
export async function generateMetadata({ params }: { params: { gtin: string } }): Promise<Metadata> {
  return {
    ...
    openGraph: {
      ...
      description: "Verified EU ESPR compliance data. Scan QR to verify factory compliance.",
      type: "website",
    }
  };
}

// Server component — reads from URL params
...
w('app/dpp/[gtin]/DppPassportClient.tsx', `
"use client";
...
// ── FIX: Update dpp page to import client component ─────────
w('app/dpp/[gtin]/page.tsx', `
import { Metadata } from "next";
...
// ── TASK 12: SEO — layout.tsx with metadata ──────────────────
w('app/layout.tsx', `
...
export default function robots(): MetadataRoute.Robots {
  return {
    ...
    sitemap: "https://dpp-atlas.vercel.app/sitemap.xml",
  };
}
`);

// ── REPORT PAGE: Add /dpp link after assessment ───────────────
// Update report page to show DPP passport link
const reportPath = 'app/[locale]/report/[id]/page.tsx';
if (fs.existsSync(reportPath)) {
  let content = fs.readFileSync(reportPath, 'utf8');
  // Add DPP link near the bottom action buttons
  const target = 'className="no-print" style={{display:"grid"';
  const replacement = `className="no-print" style={{marginBottom:"16px", background:"#f0fdfa", border:"1px solid #0d9488", borderRadius:"12px", padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"8px"}}>
          <div>
  ...
  if (content.includes(target)) {
    ...
    console.log('OK: report page updated with DPP passport link');
  }
}

console.log('');
console.log('=== ALL TASK 10 + 12 FILES WRITTEN ===');
console.log('');
console.log('Now run: git add . && git commit -m "Task 10: Public DPP passport + Task 12: SEO" && git push');