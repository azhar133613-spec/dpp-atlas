...
// Fixed assess page - voice input with strict TypeScript
...
const QS = [
  ...
  {id:3,  cat:1, q:"What is your Supply Chain Tier Level? (Informational)",                  qBn:"আপনার সাপ্লাই চেইন স্তর কী? (তথ্যমূলক)",
    opts:[{l:"Tier 1 - Garment",lBn:"Tier 1",v:"t1",s:0},{l:"Tier 2 - Fabric/Dye",lBn:"Tier 2",v:"t2",s:0},{l:"Tier 3 - Yarn",lBn:"Tier 3",v:"t3",s:0},{l:"Tier 4 - Raw Fiber",lBn:"Tier 4",v:"t4",s:0}], info:true},
  {id:4,  cat:1, q:"Does your factory have a verified tax identification number (TIN/BIN)?", qBn:"আপনার কারখানার কি যাচাইকৃত TIN/BIN আছে?",
    opts:[{l:"Yes",lBn:"হ্যাঁ",v:"y",s:5},{l:"No",lBn:"না",v:"n",s:0}]},
  {id:5,  cat:1, q:"Is your factory location verified on an official business registry?",    qBn:"আপনার কারখানার অবস্থান কি সরকারি রেজিস্ট্রিতে যাচাইকৃত?",
    opts:[{l:"Yes",lBn:"হ্যাঁ",v:"y",s:5},{l:"No",lBn:"না",v:"n",s:0}]},
  ...
  {id:7,  cat:2, q:"Are all fiber sources traceable to their country/region of origin?",    qBn:"সব ফাইবার উৎস কি উৎপত্তি দেশে ট্রেস করা যায়?",
  ...
  {id:9,  cat:2, q:"Do you hold a valid OEKO-TEX certification?",                           qBn:"আপনার কি বৈধ OEKO-TEX সার্টিফিকেট আছে?",
    opts:[{l:"Yes, current",lBn:"হ্যাঁ, বর্তমান",v:"y",s:5},{l:"Expired",lBn:"মেয়াদোত্তীর্ণ",v:"p",s:2},{l:"No",lBn:"না",v:"n",s:0}]},
  {id:10, cat:3, q:"Has your facility completed a REACH compliance audit?",                 qBn:"আপনার কারখানা কি REACH অডিট সম্পন্ন করেছে?",
...
export default function AssessPage({ params }: { params: { locale: string } }) {
  ...
  const startVoice = useCallback((qId: number) => {
    ...
    r.onstart  = () => { setListening(true); setActiveVoice(qId); };
    r.onend    = () => { setListening(false); };
    r.onerror  = () => { setListening(false); };
    r.onresult = (e) => {
      const txt = e.results[0][0].transcript;
      setVoiceNotes(prev => ({...prev, [qId]: txt}));
    };
    r.start();
    recogRef.current = r;
  }, [isBn]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const report   = {
      ...
      answers, factory, failed_ids:failedIds, voice_notes:voiceNotes,
  ...
  if (step === 0) return (
    <main style={{minHeight:"100vh", background:"#f8fafc", fontFamily:"system-ui,sans-serif", padding:"24px", display:"flex", alignItems:"center", justifyContent:"center"}}>
      <div style={{maxWidth:"680px", width:"100%", textAlign:"center"}}>
        ...
        {voiceOK && (
          <div style={{background:"#f0fdfa", border:"1px solid #99f6e4", borderRadius:"10px", padding:"10px 16px", marginBottom:"20px", fontSize:"0.82rem", color:"#0f766e", display:"inline-flex", gap:"8px", alignItems:"center"}}>
            🎤 {isBn ? "ভয়েস ইনপুট চালু আছে — প্রতিটি প্রশ্নে মাইক বাটন ব্যবহার করুন" : "Voice input enabled — use the mic button on each question"}
          </div>
        )}
        {!voiceOK && (
          <div style={{background:"#fef3c7", border:"1px solid #fbbf24", borderRadius:"10px", padding:"10px 16px", marginBottom:"20px", fontSize:"0.82rem", color:"#92400e", display:"inline-flex", gap:"8px", alignItems:"center"}}>
            🎤 {isBn ? "ভয়েস ইনপুটের জন্য Chrome বা Edge ব্যবহার করুন" : "Voice input requires Chrome or Edge browser"}
          </div>
        )}
    ...
    <main style={{minHeight:"100vh", background:"#f8fafc", fontFamily:"system-ui,sans-serif", padding:"24px"}}>
      ...
      <div style={{maxWidth:"720px", margin:"0 auto"}}>
        ...
        <div style={{display:"grid", gap:"20px", marginBottom:"28px"}}>
          {catQs.map((q, qi) => {
            const isActive = activeVoice === q.id && listening;
            return (
              <div key={q.id} style={{background:"white", border:"1.5px solid "+(answers[q.id] !== undefined ? "#0d9488" : "#e2e8f0"), borderRadius:"14px", padding:"20px"}}>
                ...
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"10px", marginBottom:"14px"}}>
                  ...
                  {voiceOK && (
                    <button
                      onClick={() => isActive ? stopVoice() : startVoice(q.id)}
                ...
                {isActive && (
                  <div style={{background:"#fee2e2", border:"1px solid #fecaca", borderRadius:"8px", padding:"8px 12px", marginBottom:"10px", fontSize:"0.8rem", color:"#dc2626", display:"flex", alignItems:"center", gap:"6px"}}>
                    ...
                    {isBn ? "শুনছি... বাংলায় বা ইংরেজিতে বলুন" : "Listening... speak now"}
                  </div>
                )}

                {voiceNotes[q.id] && !isActive && (
                  <div style={{background:"#f0fdfa", border:"1px solid #99f6e4", borderRadius:"8px", padding:"8px 12px", marginBottom:"10px", fontSize:"0.8rem", color:"#0f766e", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <span>🎤 {voiceNotes[q.id]}</span>
                    <button onClick={() => setVoiceNotes(prev => { const n={...prev}; delete n[q.id]; return n; })}
                      style={{background:"none", border:"none", color:"#94a3b8", cursor:"pointer", fontSize:"0.875rem"}}>✕</button>
                  </div>
                )}
...
console.log('');
console.log('=== DONE ===');
console.log('Now run: git add . && git commit -m "Task 6: Voice input fixed + Bengali + news API" && git push');